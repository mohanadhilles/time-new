...
class EmployeeController extends Controller
{
    ...
    public function deActive(Employee $employee)
    {
        ...
        $emp_id = $employee->first();
        if (isset($emp_id) and $emp_id->is_active == 1) {
            User::where('id', $emp_id->user_id)->update(['is_active' => 0]);
            Employee::where('id',$emp_id->id)->update(['is_active' => 0]);
            return redirect()->back()->with('success', __('Done Disabled.'));

        }else {
            User::where('id', $emp_id->user_id)->update(['is_active' => 1]);
            Employee::where('id',$emp_id->id)->update(['is_active' => 1]);
            return redirect()->back()->with('success', __('Done Enabled.'));
        }
    }
}
